...
describe('friends', () => {
  ...
  let intPublicKeyUser1 = parseInt(
    Buffer.from(bs58.decode(user1.publicKey.toBase58())).toString('hex'),
    16,
  )
  let intPublicKeyUser2 = parseInt(
    Buffer.from(bs58.decode(user2.publicKey.toBase58())).toString('hex'),
    16,
  )
  ...
  let intPublicKeyUser3 = parseInt(
    Buffer.from(bs58.decode(user3.publicKey.toBase58())).toString('hex'),
    16,
  )
  let intPublicKeyUser4 = parseInt(
    Buffer.from(bs58.decode(user4.publicKey.toBase58())).toString('hex'),
    16,
  )
  ...
  it('User 1 creates a new request for user 2 (payer user 1)', async () => {
    ...
    const requestAccountsFriendRemoved =
      await program.account.friendRequest.all([
        {
          memcmp: {
            offset: 8 + 32,
    ...
    const requestAccountsRequestRemoved =
      await program.account.friendRequest.all([
        {
          memcmp: {
            offset: 8 + 32,
  ...
  it('Creates a new request from user 3 to user 4 (payer other User)', async () => {
    ...
    let requestAccount = await program.account.friendRequest.fetch(
      newRequest[0],
    )
    const requestAccountsAll = await program.account.friendRequest.all()
    const requestAccountsPending = await program.account.friendRequest.all([
  ...
  it('User 5 cannot create a request for user 6 if public key (numeric value) of user 5 is less then public key of user 6', async () => {
    ...
    let intPublicKeyUser5 = parseInt(
      Buffer.from(bs58.decode(user5.publicKey.toBase58())).toString('hex'),
      16,
    )
    let intPublicKeyUser6 = parseInt(
      Buffer.from(bs58.decode(user6.publicKey.toBase58())).toString('hex'),
      16,
    )
